  ...
  <?php } ?>
    ...
    <table class="programData">
      <tr>
        ...
        <td>
          <?php print $content['field_taxonomy_region'] ? '<h6>' . render($content['field_taxonomy_region']) . '</h6>' : ''; ?>
...
<div class="wrapper">
      ...
      <?php
        if(isset($content['field_title_featured_stories'])) {
          if(isset($content['field_featured_stories_link'])) {
            global $language;
            $featured_stories_title = strip_tags(render($content['field_title_featured_stories']));
            $featured_stories_link = render($content['field_featured_stories_link']);
            ...
            print l($featured_stories_title . $icon, $featured_stories_link, array('html' => true));
          } else {
  ...
  <?php } 
    if(in_array('more_about', $content['block_list']) && isset($content['field_more_about'])) {
  ?>
  <!--/aside -->
  <div>
   <h5 class="preTitle">
    <?php 
      if(isset($content['field_more_about_title'])) {
        if(isset($content['field_more_about_link']) && strip_tags(render($content['field_more_about_link'])) != null) {
          $abouttitle = strip_tags(render($content['field_more_about_title']));
          ...
          print l($abouttitle . '&nbsp' . $icon, $aboutlink, array('html' => true, 'attributes' => array('title' => $abouttitle)));
        } else {
          print render($content['field_more_about_title']);
        } 
      } else {
        print html_entity_decode($title, ENT_QUOTES);
      }
      ?>
    </h5>
