...
<aside class="opportunity <?php print $content['field_block_class']; ?>">
  <h4><?php print render($title); ?></h4>
    <dl>
    <?php
      $type = 'all';
      if (isset($settings[0]['type'])) {
        $type = implode('+', $settings[0]['type']);
      }
      if (isset($settings[1]['tags'])) {
        foreach ($settings[1]['tags'] as $tag) {
          $term = taxonomy_term_load($tag);
          $result = count(views_get_view_result('agency_taxonomy_term','default', $term->tid, $type));
          if (isset($term->tid) && $result) {
            print '<dd>' . l($term->name, 'taxonomy/term/' . $term->tid . '/' . $type) .'</dd>';
          }
        }
      }
    ?>
